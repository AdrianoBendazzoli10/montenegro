...
app = FastAPI(
    title="Montenegro API",
    version="2.0.0",
)


# pasta onde as capas das obras serão salvas
...
# permite acessar as imagens salvas pelo navegador
app.mount(
    "/uploads",
    StaticFiles(
...
def normalize_worth_it(value: str) -> str:
    ...
    if result not in {
        ...
        raise HTTPException(
            status_code=400,
            detail="Resposta de valeu a pena inválida.",
        )
...
def list_works(
    ...
    return {
        "works": [
            ...
            for work, avg_rating, review_count
            in rows
        ]
    }
...
def create_work(
    ...
    try:
        ...
        raise HTTPException(
            status_code=409,
            detail="Essa obra já está cadastrada.",
        )
...
@app.delete(
    "/shelves/{shelf_id}/items/{work_id}",
